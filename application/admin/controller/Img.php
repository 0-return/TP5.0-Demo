...
class Img extends Init
{
    ...
    {
        $path = ROOT_PATH.strtolower($this->config['upload']).DS.'images'.DS.date('Y-m-d',time());
        $info = $files->validate(['size'=>10240000,'ext'=>'jpg,png,gif'])->rule('uniqid')->move($path);
        if ($info)
        {
            $img['info'] = $info->getInfo();
            $img['path'] = DS.$this->config['upload'].DS.'images'.DS.date('Y-m-d',time()).DS.$info->getSaveName();
            return $img;
        }else{
