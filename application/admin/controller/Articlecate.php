...
class Articlecate extends Init
{
    function _initialize()
    {
        ...
        $this->table = $this->config['prefix'] . 'article_cate';
    }
    ...
    public function _before_add(&$list)
    {
        if ($this->request->post()) {
            ...
            $option = $this->cate_tree_html($this->obj[1], $this->table);
    ...
    public function _before_edit(&$list)
    {
        if ($this->request->post()) { } else {
            $w['id'] = $_REQUEST['id'];
            $tid = $this->obj[1]->table($this->table)->where($w)->value('tid');
            $option = $this->cate_tree_html($this->obj[1], $this->table, '-1', 0, $tid);
            $this->assign('option', $option);
        }
    }
