...
class Admin extends Init
{
    ...
    public function _initialize()
    {
        ...
        parent::_init();
        $this->table = $this->config['prefix'].'admin';
        $where['id'] = getUser($this->request->module())['id'];
        $user = $this->obj->table(strtolower($this->config['prefix'].'admin'))->where($where)->field('num,preip,pretime')->find();
        $where['id'] = getUser($this->request->module())['id'];
    ...
    {
        $res = $this->obj->table($this->config['prefix'].'role')->select();
    ...
    public function takMenu()
    {
        ...
        $where['uid'] = $user['id'];
        $ids = $this->obj->table($this->config['prefix'].'taskbar')->where($where)->value('acids');unset($where);
        ...
        foreach ($ids_arr as $key => $value)
        {
            ...
            $res = $this->obj->table($this->config['prefix'].'auth')->where($where)->order('sort asc')->find();
            if ($res)
            {
                $data[] = $res;
            }
    ...
    {
        $user = json_decode(Cookie::get($this->request->module().'_info'),1);
        $ids = getFields($this->obj->table($this->config['prefix'].'role'),"id = {$user['roleid']}",array('fields'=>'rules','type' => 'value'));
